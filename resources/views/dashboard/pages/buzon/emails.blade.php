...
@section('content')
    @include('dashboard.components.creacion.modal_email')
    @include('dashboard.components.estado.modal_email')
        ...
        <table class="w-full text-sm text-left rtl:text-right shadow-md">
            <thead class="text-xs text-white uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                <tr>
                    ...
                    <th scope="col" class="px-6 py-3">
                        <a
                            href="{{ route('emails', array_merge(request()->query(), ['order_by' => 'estado', 'order_direction' => request('order_direction') == 'asc' ? 'desc' : 'asc'])) }}">
                            Estado
